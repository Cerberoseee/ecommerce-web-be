...
const getOrderHistory = async (req, res, next) => {
    try {
        const { productId, timePeriod, limit = 20 } = req.query;
        ...
        if (timePeriod) {
            ...
            switch (timePeriod) {
                ...
                case 'lastyear':
                    ...
                    break;
                default:
                    // Default to last 30 days if invalid time period
                    startDate.setDate(startDate.getDate() - 30);
            }
        ...
        const orders = orderItems.map(item => {
            return {
                ...
                date: item.orderId ? item.orderId.createdAt : null,
                quantity: item.quantity,
                unitPrice: item.price,
                total: item.quantity * item.price,
                status: item.orderId ? item.orderId.status : 'unknown'
            };
        ...
        res.status(200).json({
            success: true,
            message: `Order history for product ${productId}`,
            orders: orders,
            analytics: {
        ...
        return next(new AppError(`Error getting order history: ${error.message}`, 500));
    }
};
